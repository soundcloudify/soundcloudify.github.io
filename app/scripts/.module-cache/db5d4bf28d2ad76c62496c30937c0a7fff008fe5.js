		...
		return (
			React.createElement("li", {id: 'track-item-' + track.id, className: "track-item"}, 
				...
				React.createElement("div", {className: "md-tile-hover"}, 
			        starButton, 
			        React.createElement("i", {className: "add-to-playlist-btn icon ion-log-in", title: "Play Next", onClick: this.onAddTrackToPlaylist}), 
...
window.TrackList = React.createClass({displayName: "TrackList",
	...
	render: function() {
		...
		var rows = _.map(tracks, function(track) {
			...
			if (track) {
				return (
					React.createElement(TrackItem, {key: track.uuid, track: track, trackNumber: trackNumber, player: player, onTrackClick: onTrackClick, onAddTrackToPlaylist: onAddTrackToPlaylist, onRemoveTrack: onRemoveTrack, showRemoveButton: showRemoveButton, onStarTrack: onStarTrack, onPlayNext: onPlayNext})
				);
			}
		});
