...
window.TrackItem = React.createClass({displayName: "TrackItem",
	...
	render: function() {
		...
		if (this.props.showRemoveButton) {
			removeButton = (
                React.createElement("i", {className: "remove-btn icon ion-ios-close-empty", onClick: this.onRemoveTrack, title: "Remove"})
			);
		}
		...
		if (track.starred) {
			...
			starButton = (
				React.createElement("i", {className: "like-btn icon ion-ios-star", title: "Star", onClick: this.onStarTrack})
			)
		}
		...
		return (
			React.createElement("li", {id: 'track-item-' + track.id, className: "track-item"}, 
				...
				React.createElement("div", {className: "md-tile-hover"}, 
			        starButton, 
			        React.createElement("i", {className: "add-to-playlist-btn icon ion-ios-paperplane", title: "Play Next", onClick: this.onAddTrackToPlaylist}), 
			        React.createElement("i", {className: "add-to-playlist-btn icon ion-ios-plus-empty", title: "Add to playlist", onClick: this.onAddTrackToPlaylist}), 
				    removeButton
				)
			)
		);
